...
def _effect_encore(attacker: pk.Pokemon, defender: pk.Pokemon, battlefield:
    bf.Battlefield, battle: bt.Battle, move_data: Move, is_first: bool,
    cc_ib: list) ->bool:
    if (defender.is_alive and not defender.encore_count and defender.
        last_move and defender.last_move.cur_pp and defender.last_move not in
        gd.ENCORE_CHECK and any([(move.name == defender.last_move.name) for
        move in defender.moves])):
