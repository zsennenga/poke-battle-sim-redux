...
def _effect_dig(attacker: pk.Pokemon, defender: pk.Pokemon, battlefield: bf
    .Battlefield, battle: bt.Battle, move_data: Move, is_first: bool, cc_ib:
    list) ->bool:
    if not move_data.ef_stat and not _power_herb_check(attacker, battle):
        ...
        battle.add_text(attacker.nickname +
            ' burrowed its way under the ground!')
        return True
