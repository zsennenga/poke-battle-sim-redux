...
def _effect_thief(attacker: pk.Pokemon, defender: pk.Pokemon, battlefield:
    ...
    if (defender.item and dmg and not attacker.item and not defender.
        substitute and not defender.has_ability(Ability.STICKY_HOLD) and not
        defender.has_ability(Ability.MULTITYPE)):
        battle.add_text(attacker.nickname + ' stole ' + defender.nickname +
            "'s " + cap_name(defender.item) + '!')
